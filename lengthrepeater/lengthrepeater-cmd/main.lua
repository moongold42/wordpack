...
print("be sure that you are running this script in cmd!")
Idx = 0
Timeidx = 1
...
function fileIsExistent(filename)
...
os.execute("ffmpeg -f concat -i arrayy.txt -c:v copy -c:a copy output.mkv")
os.execute("del /q tempry\\*")
os.execute("type nul > array.txt")
